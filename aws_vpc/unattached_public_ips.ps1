...
Write-Output ""
Write-Output "VPC Unattached Public IPs"
...
#Set CSV Headers
"""PublicIP"",""AllocationID"",""Domain"",""Region""" | Out-File -Encoding ASCII -FilePath ".\aws_unattached_elastic_ips.csv"

#Get Unattached Elastic IPs Volumes
Write-Host "[$(get-date -Format "dd/mm/yy hh:mm:ss")] Gathering Unattached Elastic IP Addresses..."
foreach ($region in Get-AWSRegion) {
  Write-Host ("[$(get-date -Format "dd/mm/yy hh:mm:ss")] Processing Region - "+$region.Region)
    foreach ($eip in Get-EC2Address -Region $region.Region | Where-Object {$_.AssociationId -eq $null}) {
        """" + $eip.PublicIp + """,""" + $eip.AllocationId + """,""" + $eip.Domain + """,""" + $region.Region + """" | 
            Out-File -Encoding ASCII -FilePath ".\aws_unattached_elastic_ips.csv" -Append
    }
